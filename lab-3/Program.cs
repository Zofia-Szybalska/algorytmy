...
namespace lab_3 {
    class Program {
        static void Main(string[] args) {
            Item[] test = new Item[] { 
                new Item {Wartosc = 10, LiczbaPrzegrodek = 1 },
                new Item {Wartosc = 7, LiczbaPrzegrodek = 2 },
                new Item {Wartosc = 3, LiczbaPrzegrodek = 1 },
                new Item {Wartosc = 8, LiczbaPrzegrodek = 1 },
            };
            Console.WriteLine(zadanie2(test, 3));
        }
        ...
        static int zadanie2(Item[] przedmioty, int liczbaPrzegrodekWPlecaku) {
            int suma = 0;
            Dictionary<int, double> wartosci = new Dictionary<int, double>();
            for (int i = 0; i < przedmioty.Length; i++) {
                wartosci.Add(i, (double)przedmioty[i].Wartosc / (double)przedmioty[i].LiczbaPrzegrodek);
            }
            ...
            for (int i = 0; i < przedmioty.Length; i++) {
                ...
                else if (liczbaPrzegrodekWPlecaku == 1 && i >=1){
                    if (przedmioty[wartosciPosortowane[i].Key].LiczbaPrzegrodek == 2 && przedmioty[wartosciPosortowane[i-1].Key].LiczbaPrzegrodek == 1 && przedmioty[wartosciPosortowane[i].Key].Wartosc > przedmioty[wartosciPosortowane[i - 1].Key].Wartosc)  {
                        suma -= przedmioty[wartosciPosortowane[i-1].Key].Wartosc;
                        suma += przedmioty[wartosciPosortowane[i].Key].Wartosc;
                        liczbaPrzegrodekWPlecaku--;
                    }
                    /* if (przedmioty[wartosciPosortowane[i].Key].LiczbaPrzegrodek == 1 && przedmioty[wartosciPosortowane[i - 1].Key].LiczbaPrzegrodek == 2 && przedmioty[wartosciPosortowane[i + 1].Key].LiczbaPrzegrodek == 1 && przedmioty[wartosciPosortowane[i].Key].Wartosc + przedmioty[wartosciPosortowane[i + 1].Key].Wartosc > przedmioty[wartosciPosortowane[i - 1].Key].Wartosc) {
                        suma -= przedmioty[wartosciPosortowane[i - 1].Key].Wartosc;
                        suma += przedmioty[wartosciPosortowane[i].Key].Wartosc + przedmioty[wartosciPosortowane[i + 1].Key].Wartosc;
                    }*/
                }
                if (liczbaPrzegrodekWPlecaku==0) { break; }
            }
            return suma;
        }
        class CasheRegister {
